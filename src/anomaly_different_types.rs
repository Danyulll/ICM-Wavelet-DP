...
// Apply different anomaly types to different channels
pub fn apply_different_anomalies_to_curve(
    rng: &mut StdRng,
    curve: &mut CurveM,
    t: &Array1<f64>,
    anomaly_types: &[AnomType], // Different anomaly type for each channel
) {
    let n = t.len();
    let m = curve.y.shape()[1];

    // Apply different anomaly type to each channel
    for a in 0..m {
        let anomaly_type = &anomaly_types[a % anomaly_types.len()];
...
// Create single anomaly dataset with different anomaly types per channel
pub fn make_single_anomaly_dataset_different_types(
    rng: &mut StdRng,
    n_curves: usize,
    anomaly_combination: &[AnomType], // Combination of anomaly types for different channels
) -> LabeledDatasetM {
    ...
    // create anomaly curves from normal model then apply different anomalies per channel
    let mut anom_curves = simulate_icm_curves(rng, n, m_out, &kx_norm, &b_norm, &eta_norm, n_anom);
    for curve in anom_curves.iter_mut() {
        apply_different_anomalies_to_curve(rng, curve, &t, anomaly_combination);
    }
    ...
    // For this experiment, we'll label the entire curve as "Mixed" since it has mixed anomaly types
    let mut labels = vec![AnomType::Normal; n_norm];
    labels.extend(vec![AnomType::Decouple; n_anom]); // Use Decouple as a generic "mixed anomaly" label
...
// Create all single anomaly datasets with different anomaly types per channel
pub fn make_all_single_anomaly_datasets_different_types(
    rng: &mut StdRng,
    n_curves: usize,
) -> Vec<(String, LabeledDatasetM)> {
    // Define different combinations of anomaly types for the 3 channels
    let anomaly_combinations = vec![
        // Each combination has 3 elements for the 3 channels
        vec![AnomType::Shift, AnomType::Amplitude, AnomType::Shape],
        vec![AnomType::Trend, AnomType::Phase, AnomType::Decouple],
        vec![AnomType::Smoothness, AnomType::NoiseBurst, AnomType::Shift],
        vec![AnomType::Amplitude, AnomType::Shape, AnomType::Trend],
        vec![AnomType::Phase, AnomType::Decouple, AnomType::Smoothness],
        vec![AnomType::NoiseBurst, AnomType::Shift, AnomType::Amplitude],
        vec![AnomType::Shape, AnomType::Trend, AnomType::Phase],
        vec![AnomType::Decouple, AnomType::Smoothness, AnomType::NoiseBurst],
    ];
    
    anomaly_combinations.iter().enumerate().map(|(i, combo)| {
        let ds = make_single_anomaly_dataset_different_types(rng, n_curves, combo);
        let combo_name = combo.iter()
            .map(|t| crate::anomaly::anom_slug(t))
            .collect::<Vec<_>>()
            .join("_");
        (format!("combo_{}_{}", i + 1, combo_name), ds)
    }).collect()
}
